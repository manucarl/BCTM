                     ...
                     thetaseq = NULL, type = "SD", truncate = 0.95)
{
  ...
  # ct = threshold from eq. (8) in Klein & Kneib (2016)
  ...
  for(r in 1:length(omegaseq)) {
    cat("r (prep.): ", r, "\n")
    K <- (omegaseq[r]*(K1) + (1-omegaseq[r])*(K2))
  ...
  simsup <- function(theta) {
    ...
    for(r in 1:length(omegaseq)) {
      ...
      # U <- ginv(W)%*%t(V)
      # 
      # betasample <- betasample - t(U)%*%betasample
      
      # fsample <- Z%*%betasample
      supsample[ind] <- apply(abs(betasample),2, max)
    }
  ...
  if(is.null(thetaseq))
  {
    ...
    {
      quantile(simsup(theta=theta), probs=1-alpha) - ct
    }
  ...
  else
  {
    ...
    for(theta in 1:length(thetaseq))
    {
      ...
      res[theta] <- quantile(simsup(theta=thetaseq[theta]), probs=1-alpha) - ct
    }
  }
  return(res)
}
