...
# prediction df
pred_grid <-expand.grid(cholst=y_pred, age=age_pred, year = year_pred, sex=0)
...
X_pred <- cbind(1, B1_pred, B2_pred, pred_grid[,c("sex", "year")]) %>% as.matrix
# derivative of prediction design matrix
Xp_pred <-  cbind(0, Bp1_pred, Bp2_pred,  0, 0) %>% as.matrix
