// posterior and gradient functions for Gaussian, logistic and minimu-extreme-value reference distribution
...
vec gradf_mev(const arma::vec &params, const Rcpp::List &xx){
  ...
  // rowvec  = rowvec(1-eh);
  vec res = rowSums(CXt.each_row()%((1-eh)-(X*bt_vec - eh )).t() + XpC.each_row()%w.t())- p3;
...
double posterior_mev_cens(const arma::vec &params, const Rcpp::List &xx) {
  ...
  arma::vec ll2 = log(1 - pmev(as<NumericVector>(wrap(X2*bt_vec)) ) + 10e-10);
  double lp = sum(ll1) + sum(ll2)- as_scalar(0.5*sparams.t()*(S*sparams));
...
// [[Rcpp::export]]
double ll_mev_cens(const arma::vec &params, const Rcpp::List &xx) {
...
vec gradf_mev_cens(const arma::vec &params, const Rcpp::List &xx){
  ...
  vec par =  -dmev2(as<NumericVector>(wrap(X2*bt_vec)))/(1-pmev(as<NumericVector>(wrap(X2*bt_vec))) + 10e-10);
