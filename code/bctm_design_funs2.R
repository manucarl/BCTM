...
hyx_sm <- function(..., q = c(10,10) , data, knots = list(NULL, NULL), 
                   pen_order = c(2,2), degree = c(3, 3), center = T, add_to_diag = 10e-6,  scale_dep = F)
{
...
# nonlinear monotone transformations with linear VCM -----------------------------------------------------------------------------------
...
# nonlinear monotone transformations ------------------------------------------------------------------------------------------
hx_sm <- function(..., q = 20,  knots = NULL, data,
                  pen_order = 2, degree = 3, center = T)
{
  ...
  ret <- list(B=B, Bp = Bp, K = list(K), exp_ident = rep(F, ncol(B)), label = label, label_inds = rep(label, NCOL(B)), knots = knots, pen_ind = TRUE,
              rank = rank, predvars = list(B = pred_B, Bp = pred_Bp))
  # attr(ret, "pen_ind") <- F
...
hyx_lin  <- function(y, x){
  
  B <- matrix(y*x, ncol=1)
  label <- paste("hyx_lin(", match.call()$y, ",",  match.call()$x, ")", sep = "")
  colnames(B) <- label
  ...
  unq <- !duplicated(x)
  xu <- x[unq]
  construct_Bpred <- function(ynew, xnew) {
  ...
  construct_Bpredp <- function(ynew, xnew) {
    # if (any(xnew < rng[1] | xnew > rng[2])) {
    #   warning("predictions outside fitted range for ", 
    #           label, ".")
    # }
  ...
  ret <- list(B = B, Bp = Bp, label = label, label_inds = rep(label, NCOL(B)), K = list(K), exp_ident =F, pen_ind = F,
...
hy_lin  <- function(y){
  ...
  B <- matrix(y, ncol=1)
  label <- paste("hy_lin(", match.call()$y, ")", sep = "")
  colnames(B) <- label
  
  Bp <- matrix(1, length(y), ncol=1)
  K <- structure(matrix(0), rank = 0, label = label)
  ...
  construct_Bpredp <- custom_fun_env(construct_Bpredp, rng = rng, label = label, xu = xu)
  ret <- list(B = B, Bp = Bp, label = label, label_inds = rep(label, NCOL(B)), K = list(K), exp_ident = T, pen_ind = FALSE,
              predvars = list(B = construct_Bpred, Bp = construct_Bpredp))
  # attr(ret, "pen_ind") <- T
  ret
}
...
# simple linear effects ----------------------------------------------------------------------------------------------------------
hx_lin2  <- function(x, scale = F, center = F){
...
hx_re <- function(g, data){
  
var <- deparse(match.call()$g)
  
B <- model.matrix(~as.factor(data[,var])-1)[,-1] 
Bp <- matrix(0, nrow(B), ncol(B))
...
label <- paste("hre(", deparse(match.call()$g),")", 
               sep = "")
colnames(B) <- paste0("re_",sort(unique(data[,var]))[-1])

pred_B <- function(newdata) {
  B_pred<- matrix(0, nrow(newdata), ncol(B))
  newdata
}
pred_B <- custom_fun_env(pred_B)


pred_Bp <- function(newdata) {
  Bp_pred<- matrix(0, nrow(newdata), ncol(B))
}
pred_Bp <- custom_fun_env(pred_Bp)
...
hx_spat2 <- function(s, nmat = NULL, data){
  ...
  K <- matrix(unlist(nmat), nreg,nreg)[-1,-1]
  # attr(nmat, "class") <- "mat"
