...
ll_mev_i <- function(param, xx){
  bt <- param
  exp_ident <- xx$exp_ident+1
  bt[exp_ident] <- exp(bt[exp_ident])
  c(ldmev(xx$X1%*%bt) + log(xx$Xp1%*%bt), log(1-pmev(xx$X2%*%bt)))
}
...
ll_logit_i <- function(param, xx){
  bt <- param
  exp_ident <- xx$exp_ident+1
  bt[exp_ident] <- exp(bt[exp_ident])
  c(dlogis(xx$X1%*%bt, log=T) + log(xx$Xp1%*%bt), log(1-plogis(xx$X2%*%bt)))
}
