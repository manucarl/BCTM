...
##                    scale dependent prior for tau2; updates via IWLS sampler
##                    imitates the output style of rstan (Stan Development Team, 2020) and uses mass matrix adaptation from adnuts (Monnahan and Kristensen, 2018)
##                    Uses scale-dependent priors
## Author: BLIND
##
...
# 3) determine fraction of examples where max(beta_j) > c for an estimate of the prob
  ...
  for(k in 1:length(Ks_f)){
    
    # thetas[k] <- hyper_beta(ginv(Ks_f[[k]]), alpha=alpha, R=R, ct=ct, myseed=myseed)$root
    
    thetas[k] <- hyper_beta(diag(ncol(Ks_f[[k]])), alpha=alpha, R=R, ct=ct, myseed=myseed)$root
    
  }
  # K_sum <- Ks_f[[1]] + Ks_f[[2]]
  # thetas <- c(0.1, 0.1)
  
  print(paste0("elicited thetas: ", thetas))
