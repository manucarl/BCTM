##
## Script name: nuts_omega_spat_leuk_ph.R
##
...
NUTS <- function(n_iter, xx, f, gr, ll, start, warmup = floor(n_iter/2),thin=1, burnin = floor(n_iter/2),
  ...
  K_spat <- Klist[startsWith(names(Klist), "hs")][[1]][[1]]
  rank_spat <- rankMatrix(K_spat)[]
  ...
  for(iter in 1:n_iter){
    ...
    # update tau2 for h_y and multiply with corresponding precision matrix
    i <- 1
    grp <- pen_ident[[i]] 
    ...
    # update tau2 for h_spat
    i <- 2
    grp <- pen_ident[[i]] 
    par <- beta_out[iter, grp ]
    
    tau2_out[iter,i-1] <-  tau2 <- rinvgamma(1, hyper_a + 0.5*rank_spat, hyper_b + as.vector(0.5*t(par)%*%(K_spat%*%par)))
    
    Ks_t[[i]] <- Ks_f[[i]]/ tau2
    
    
    S <- as.matrix(bdiag(list(Ks_t[[1]], Ks_t[[2]])))
    xx$S <- S
  ...
  list(beta = beta_out, tau2 = tau2_out, omega = omega_out, log_liks = log_liks, lp = log_posteriors, sampler_params = sampler_params,
