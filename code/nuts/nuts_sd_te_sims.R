...
NUTS <- function(n_iter, xx, f, gr, ll, start, warmup = floor(n_iter/2),thin=1,
                 seed = NULL,  chain = 1, nuts_settings, fixed = NULL, theta = 1,
                 prior_settings,
                 sd_params=NULL){
  ...
  K_inds <- xx$K_inds
  K_sums <- lapply(K_inds, function(x) Reduce("+", Ks_t[x]))
  ...
  K_sum_list <- lapply(K_inds, function(x) Reduce("+", Ks_t[x]))
  
  # number of effects
  n_eff <- length(xx$K_inds)
  ...
  # takes e.g. K1_1 and K1_2 as list as input
  # outputs
  get_det_rank <- function(Ks){
    
    # takes e.g. K1_1 and K1_2 as list as input
    # outputs list of determinants, ranks and alternative representation of the precision with omega

  pre_Ks <- lapply(omega_grid, function(omega) Ks[[1]]*omega + (1 - omega) * Ks[[2]])
  K_determinants <- sapply(pre_Ks, function(K) det(K))
  K_ranks <- sapply(pre_Ks, rankMatrix)
  
  list(determinants = K_determinants, ranks = K_ranks, pre_Ks = pre_Ks)
  }
  
  
  K_det_ranks <- vector("list", n_eff)
  
  for(j in 1:n_eff){
    Ks <- Ks_t[K_inds[[j]]]
    
    K_det_ranks[[j]] <- get_det_rank(Ks)
    
    thetas[j] <- hyper_omega_beta(K1 = Ks[[1]], K2 = Ks[[2]], alpha=alpha, R=R, ct=ct, A=NULL,
                     omegaseq=omega_grid, omegaprob= rep(1/n_omega, n_omega), 
                     myseed=myseed)
  
    
  }
  print(paste0("theta: ", thetas))  
  

  # K1 <- Ks_f[[1]] + Ks_f[[2]]
  # K2 <- Ks_f[[3]] + Ks_f[[4]]
  # pre_Ks1 <- lapply(omega_grid, function(omega) Ks[[1]]*omega + (1 - omega) * Ks[[2]])
  # pre_Ks2 <- lapply(omega_grid, function(omega) Ks[[3]]*omega + (1 - omega) * Ks[[4]])
  # 
  # K_determinants1 <- lapply(pre_Ks1, function(K) det(K1))
  # K_determinants2 <- lapply(pre_Ks2, function(K) det(K2))
  # 
  # K_ranks1 <- lapply(pre_Ks1, rankMatrix)
  # K_ranks2 <- lapply(pre_Ks2, rankMatrix)
  ...
  omega_out <- matrix(0.5, nrow=n_iter, ncol=n_eff)
  colnames(omega_out) <- unlist(  lapply(1:n_eff, function(x) paste0("omega_", x)))
  
  
  K_updated <- vector("list", n_eff)
  ...
  for(iter in 1:n_iter){
    ...
    #   
    #   tau2 <- if(iter == 1) start_tau else tau2_out[iter - 1, i]
    #   uc <- log(tau2)
    #   
    #   K <- K_sum
    #   theta <- thetas[i]
    ...
    #   
    #   
    #   u_proposal <- rnorm(1, mu_uc, sqrt(sigma2_uc))
    #   
    ...
    #     tau2_out[iter, i] = tau2 = exp(u_proposal)
    #   }else{
    #     tau2_out[iter, i] = tau2
    #   }
    #   
    #   omega_probs <- sapply(1:n_omega, function(w) sqrt(( 1 / tau2)^K_ranks[[w]] * K_determinants[[w]]) * exp(- 1/(2*tau2) * (t(par)%*% (pre_Ks[[w]] %*% par))) * 1 / n_omega)
...
for(j in 1:n_eff){
    ...
    tau2 <- if(iter == 1) start_tau else tau2_out[iter - 1, j]
    uc <- log(tau2)
    
    theta <- thetas[j]

    rank <- K_det_ranks[[j]]$ranks[1]
    ...
    if (runif(1) < exp(log_probab)){
      tau2_out[iter, j] = tau2 = exp(u_proposal)
    }else{
      tau2_out[iter, j] = tau2
    }
    
    print(tau2)
    omega_probs <- sapply(1:n_omega, function(w) sqrt(( 1 / tau2)^rank * K_det_ranks[[j]]$determinants[w]) * exp(- 1/(2*tau2) * (t(par)%*% ( K_det_ranks[[j]]$pre_Ks[[w]] %*% par))) * 1 / n_omega)
    min_prob <- min(omega_probs)
    omega_probs <- (omega_probs - min_prob) / (max(omega_probs - min_prob))
    
    omega <- sample(omega_grid, size=1,prob=omega_probs)

    K_updated[[j]] <- (omega * Ks[[1]] + (1-omega) * Ks[[2]])/ tau2
    
  }
    
    
    S <- as.matrix(bdiag(K_updated))
    xx$S <- S
