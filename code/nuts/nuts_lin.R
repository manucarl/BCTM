...
find_reasonable_epsilon <- function(beta, f, gr, xx, its=500){
  ...
  while (!is.finite(H) | !is.finite(H_prime) | a*H_prime-a*H > -a*log(2)) {
    ...
    H_prime <- calculate_hamiltonian(beta = beta_prime,  xx = xx, r = r_prime, f = f)
    k <- k + 1
    if(k > its) stop(paste0("Could not find reasonable epsilon in ",its, " iterations. Model may be misspecified."))
  }
